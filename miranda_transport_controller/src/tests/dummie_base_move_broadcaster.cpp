...
int main(int argc, char** argv){
  ...
  ros::NodeHandle("~").getParam("tf_prefix",tf_prefix);
  while(ros::ok())
  {
    tf2::Quaternion q;
    q.setRPY(0, 0, angle);
    transformStamped.header.frame_id=tf::resolve(tf_prefix,"odom");
    transformStamped.header.stamp=ros::Time::now();
    transformStamped.child_frame_id=tf::resolve(tf_prefix,"base_link");
