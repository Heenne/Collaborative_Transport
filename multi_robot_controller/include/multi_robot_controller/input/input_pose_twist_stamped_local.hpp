...
class InputPoseTwistStampedLocal: public InputBase
{   
    public:
        InputPoseTwistStampedLocal():InputBase()
        {;}
        InputPoseTwistStampedLocal(ros::NodeHandle &nh,std::string topic_name_pose,std::string topic_name_twist):InputBase(nh)
        {
            this->subscribe(topic_name_pose,topic_name_twist);
           
        }
        inline InputPoseTwistStampedLocal(ros::NodeHandle &nh):InputBase(nh)
        {
            ros::NodeHandle priv("~");
        ...
        inline InputPoseTwistStampedLocal(ros::NodeHandle &nh,ros::NodeHandle &nh_param):InputBase(nh)
        {
    ...
    private:
        ...
        {
            this->sub_pose_=this->nh_.subscribe(topic_name_pose,10,&InputPoseTwistStampedLocal::setPose,this);
            this->sub_twist_=this->nh_.subscribe(topic_name_twist,10,&InputPoseTwistStampedLocal::set,this);
        }
