...
class InputPoseTwistStamped: public InputBase
{   
    public:
        InputPoseTwistStamped():InputBase()
        {;}
        InputPoseTwistStamped(ros::NodeHandle &nh,std::string topic_name_pose,std::string topic_name_twist):InputBase(nh)
        {
            this->subscribe(topic_name_pose,topic_name_twist);
           
        }
        inline InputPoseTwistStamped(ros::NodeHandle &nh):InputBase(nh)
        {
            ros::NodeHandle priv("~");
        ...
        inline InputPoseTwistStamped(ros::NodeHandle &nh,ros::NodeHandle &nh_param):InputBase(nh)
        {
    ...
    private:
        ...
        {
            this->sub_pose_=this->nh_.subscribe(topic_name_pose,10,&InputPoseTwistStamped::setPose,this);
            this->sub_twist_=this->nh_.subscribe(topic_name_twist,10,&InputPoseTwistStamped::set,this);
        }
